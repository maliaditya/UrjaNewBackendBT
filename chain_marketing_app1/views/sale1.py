        ...
        if serializer.is_valid():
            ...
            if self.check_products_available_in_stock(orders,product_id):
                try:
                    ...
                    order = OrderDetails.objects.get(id= self.PRODUCT)
                    order.quant
                    order.quantity_sold -= 1
                    order.save()
                    return Response( {"message":"Failed by internal error!!!"},serializer.data, status=status.HTTP_409_CONFLICT)
            else:
