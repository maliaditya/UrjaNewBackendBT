...
class GetAllHeirachy(viewsets.ModelViewSet):
    ...
    def list(self, request):
        self.TEMP.clear()
        # starting_node = MemberAccount.objects.get(is_admin=True)
        queryset = MemberAccount.objects.get(is_admin=True)
        name = queryset.get_user_name
        self.insert(key=queryset.member_id,name=name)
        print(self.TEMP)
