...
class GetBinaryTreeHeirachy(viewsets.ModelViewSet):
    ...
    TEMP = list()
    f = open("btd.txt", "w")
    f.write("**************get_leftdata_members******************")
    
    def get_member_info(self, member_id):
        member = MemberAccount.objects.get(member_id=member_id)
        return member

    def get_left_members(self,member_id):
        directs = self.get_direct_members(member_id)
        active_left_direct = 0
        try:
            for i in directs:
                self.f.write("\n"+ i)
                self.f.close()
                # member = BinaryTree.objects.get(member=i)
                # if member.is_active and member.position=='Left':
                #     active_left_direct+=1
        except Exception as e:
        	pass
        return active_left_direct

    def get_total_left_members(self,member_id):
        directs = self.get_direct_members(member_id)
        active_left_direct = 0
        for i in directs:
            member = BinaryTree.objects.get(member=i)
    ...
    def get_right_members(self,member_id):
        directs = self.get_direct_members(member_id)
        active_right_direct = 0
        for i in directs:
            member = BinaryTree.objects.get(member=i)
            if member.is_active and member.position=='Right':
                active_right_direct+=1
        return active_right_direct
            
    def get_total_right_members(self,member_id):
        directs = self.get_direct_members(member_id)
        active_right_direct = 0
        for i in directs:
            member = BinaryTree.objects.get(member=i)
            if member.position=='Right':
                active_right_direct+=1
        return active_right_direct
    def get_direct_members(self,member_id):
        Directs = MemberAccount.objects.filter(sponser_id=member_id)
        return Directs
        #    'active_left_direct':f'{self.get_left_members(key)}/{self.get_total_left_members(key)}',
        #     'active_right_direct':f'{self.get_right_members(key)}/{self.get_total_right_members(key)}',
    ...
    def create_node(self,key,name,parent,left_points=None, right_points=None,is_valid=None,is_active=None,sponser=None):
        if self.ROOT is  None:
            ...
            self.ROOT = TreeNode(key,attributes={   'Name':name,
                                                    ...
                                                    'is_valid':is_valid,
                                                    'is_active':is_active,
                                                    'active_left_direct':self.get_left_members(key),
                                                    'active_right_direct':f'asdasd',
                                                    'sponser':self.get_sponser_of_current_member(key)
                                                })
